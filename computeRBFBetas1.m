function betas = computeRBFBetas1(X, centroids, labcenter,memberships)
...
% $Author: ChrisMcCormick $    $Date: 2014/04/08 22:00:00 $    $Revision: 1.2 $
    ...
    for (i = 1 : numRBFNeurons)
        ...
        members = X((memberships ==labcenter(i)), :);
    ...
    if (any(sigmas == 0))
        %A=find(~sigmas);%%%%% add
        %sigmas(A)=1;%%%%%%%% add
        error('One of the sigma values is zero!');%%%%%%%%%% original
        %betas=0;
        %return;
    end
    ...
    betas = 1 ./ (2 .* sigmas .^ 2);
    if any(any(isnan(betas)))|| any(any(isinf(betas)))
        betas
    end    
