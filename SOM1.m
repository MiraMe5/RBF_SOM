function [weights_matrix] = SOM1(initweight,input_data, N, M, nb_iter, eff_width_init, eff_width_time_cst, learning_rate_init, learning_rate_time_cst)
...
for i = 1:nb_iter
    ...
    for k = 1:nb_neurons     
        ...
        % neuron
        if min(neuron_position_i , winner_position_i)>= 0 & min(neuron_position_j , winner_position_j)>=0
            distance_winner_neuron =sum(sum((1-((1+min(neuron_position_i , winner_position_i))./(1+max(neuron_position_i , winner_position_i)))))+sum((1-((1+min(neuron_position_j , winner_position_j))./(1+max(neuron_position_j , winner_position_j))))));
        else
            distance_winner_neuron =sum(sum((1-((1+min(neuron_position_i , winner_position_i)+abs(min(neuron_position_i , winner_position_i)))./(1+max(neuron_position_i , winner_position_i)+abs(min(neuron_position_i , winner_position_i))))))+sum((1-((1+min(neuron_position_j , winner_position_j)+abs(min(neuron_position_j , winner_position_j)))./(1+max(neuron_position_j , winner_position_j)+abs(min(neuron_position_j , winner_position_j)))))));
        end
            
        %distance_winner_neuron =(sqrt(2-2*(sum(neuron_position_i * winner_position_i)./((sum(neuron_position_i).^2) + sqrt(sum(neuron_position_i).^2)))))+(sqrt(2-2 .* ( sum(neuron_position_j * winner_position_j)./((sum(neuron_position_j).^2) + (sum(neuron_position_j).^2)))));
        %distance_winner_neuron =((sum(neuron_position_i-winner_position_i).^2)./(sum(neuron_position_i).^2+sum(winner_position_i).^2 -sum(neuron_position_i*winner_position_i)))+((sum(neuron_position_j-winner_position_j).^2)./(sum(neuron_position_j).^2+sum(winner_position_j).^2 -sum(neuron_position_j*winner_position_j)));%JacD 75
        %distance_winner_neuron = norm((neuron_position_i - winner_position_i))+norm((neuron_position_j - winner_position_j));
        %distance_winner_neuron =sum(((neuron_position_i - winner_position_i)^2 ./ (neuron_position_i + winner_position_i))+((neuron_position_j - winner_position_j)^2 ./ (neuron_position_j + winner_position_j)));
